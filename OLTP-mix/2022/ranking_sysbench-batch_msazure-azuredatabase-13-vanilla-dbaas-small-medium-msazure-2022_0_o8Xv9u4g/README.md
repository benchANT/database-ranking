...
The following data sets contain the raw performance data and metadata of the [benchANT database ranking](https://benchant.com/ranking/database-ranking) for the OLTP mix workload created by the [sysbench](https://github.com/akopytov/sysbench) benchmark suite. 
...
The raw performance data output of the sysbench is contained in the `0_load.txt`  for the LOAD phase and in the `0_run.txt`for the RUN phase. The database ranking data only considers the RUN phase. 
